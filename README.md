...
```
```
5. Я насчитал 6 шагов при условии что ни в локальном кэше ни в DNS адрес не присутствует:
- Запрос на ДНС прописанный на клиентском ПК  - если ответ нет, то:
- Запрос на корневые ДНС -  перенаправит на ДНС .uk
- ДНС .uk - перенаправит на  ДНС co.uk
- ДНС co.uk - перенаправит на google.co.uk
- google.co.uk - вернёт IP локальному ДНС
- Локальный ДНС - вернёт IP клиенту сделавшему запрос.
```
```
